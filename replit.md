...
-   **UAE VAT Compliance System:** Optional VAT activation system aligned with UAE FTA requirements. Spas start with VAT disabled (for businesses under AED 375,000 annual revenue threshold). Once registered for VAT, spa owners can activate VAT collection through admin settings by providing their 15-digit Tax Registration Number (TRN). Features include:
    -   **VAT Activation Toggle:** Per-spa configuration with `vatEnabled` flag and TRN storage
    -   **Intelligent Invoice Types:** Automatic classification as 'full' (≥AED 10k), 'simplified' (<AED 10k), or 'standard' (VAT disabled)
    -   **TRN Management:** Stores supplier TRN (spa) and customer TRN (for B2B transactions)
    -   **5-Year Retention:** Automated retention date calculation and deletion prevention for FTA audit compliance
    -   **Tax Code Support:** SR (Standard 5%), ZR (Zero-Rated), ES (Exempt), OP (Out of Scope)
