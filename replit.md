...
-   **Admin Registration & Approval Flow:** Complete admin onboarding system with pending approval workflow. Admins register with spa name and optional business license document (`licenseUrl` field). Super admin reviews and approves applications. The `enforceSetupWizard` middleware ensures approved admins complete the streamlined 4-step setup wizard (Basic Info, Location, Business Hours, Activation) before accessing any admin features. Services, staff, policies, and inventory are managed post-activation from the admin dashboard. Middleware blocks all `/api/admin/*` routes except `/api/admin/setup/*` for admins with incomplete setup, returning `setupRequired: true` for frontend redirect handling. Super admins bypass all wizard enforcement.
-   **Setup Wizard Design:** The wizard features a clean, minimal design with progress tracking and step indicators. Contact email from registration is locked (read-only) in Basic Info step. Location step includes a dropdown of all 7 UAE emirates (Abu Dhabi, Dubai, Sharjah, Ajman, Umm Al Quwain, Ras Al Khaimah, Fujairah) plus optional latitude/longitude fields with helpful tips. Business Hours step uses time pickers matching the Settings page design for all 7 days. Activation step presents a completion summary with checklist visualization.
